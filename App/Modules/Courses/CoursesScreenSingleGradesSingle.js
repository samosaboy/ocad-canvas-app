...
import { TouchableHighlight, Linking, View, Text, ScrollView } from 'react-native'
...
export default class CoursesScreenSingleAssignments extends React.Component {
  ...
  _formatDate = (date) => {
    return moment(date, moment.ISO_8601).format('MM/DD/YYYY h:mm A')
  }
      ...
      <ScrollView>
              ...
              ? (
                <View style={styles.infoContainer}>
                  ...
                  <Text style={styles.infoDesc}>{this._formatDate(this.state.item.submitted_at)}
                    {this.state.item.late ? <Text style={{ color: Colors.error }}> (Late)</Text> : null}</Text>
                </View>
              )
              : null
          }
          ...
          this.state.item.attachments
            ? (
              this.state.item.attachments.map((attachments) => (
                <TouchableHighlight onPress={() => this._openAttachment(attachments.url)} key={attachments.id} underlayColor={Colors.transparent} style={Pages.viewContainer}>
        ...
        }
        {
          this.state.item.submission_comments.map((item) => (
            <View key={item.id} style={{ paddingTop: 20, paddingBottom: 20 }}>
              <ListItem
                roundAvatar
                hideChevron
